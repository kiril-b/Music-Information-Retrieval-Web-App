...
global.fetch = jest.fn((url) => {
  if (
    url ===
    "http://localhost:8000/tracks-library/get_tracks_pagination?offset=0&limit=15&next_page_track_id=null&track_listens_lower_bound=0&track_listens_upper_bound=0"
  ) {
...
describe("SearchLibrary Component - integration tests", () => {
  test("get details for Food - track", async () => {
    act(async () => {
      ...
      const playTrackSpy = jest
        .spyOn(handlePlayClick, "handlePlayClick")
        .mockImplementation();
  ...
  test("get similar tracks for Food - track", async () => {
    act(async () => {
      ...
      const trackField = screen.getByText("Food");
      expect(trackField).toBeInTheDocument();
